...
{:place_vault_in} = require "maps.01_Overworld"
...
node_dungeon_room_template = () =>
    area_temp = SourceMap.area_template_create {
        ...
        legend: {
            'x': {add: SourceMap.FLAG_SOLID, content: Tilesets.pixulloch.wall}
            '.': {add: SourceMap.FLAG_SEETHROUGH, content: if chance .5 then Tilesets.pixulloch.floor else Tilesets.pixulloch.floor_alt}
        }
    }
...
return MapDesc.create {
    map_label: "Pixullochia Depths"
    size: {130, 130}
    default_content: Tilesets.pixulloch.wall
    children: {
        MapNode.create {
