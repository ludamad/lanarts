...
PendingPortal = newtype {
    ...
    compile: (C, compiler) =>
        ...
        @feature.on_player_interact = (user) =>
            -- We call C\get(label) first, to potentially trigger map
            -- generation 
            -- This will ensure .feature is set on @other_portal
            destination_map = C\get(@other_portal.label)
            assert @other_portal.feature, "Connected portal should have compiled!"
            -- Transfer the user object between floors.
            Map.transfer(user, destination_map, @other_portal.feature.xy)
        return true
}
