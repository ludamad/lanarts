...
floor_plans = (rng) ->
    ...
    raw_plans = {
        [1]: {
            ...
            enemy_entries: if os.getenv("ARENA_ENEMIES") then loadstring(os.getenv("ARENA_ENEMIES"))() else {
--                {enemy: "Mana Sapper", guaranteed_spawns: 5, chance: 100}
            ...
            item_groups: {{ItemGroups.enchanted_items, 30}}
            number_regions: 5
            room_radius: () -> 20 -- random(2,10)
            arc_chance: 0
            n_stores: 4
            ...
            if connector.n_portals("up") == 0 -- Player spawn
                for i =1,1
                    ...
                    if not NewDungeons.place_feature(map, vault)
                        return nil
                for i =1,5
                    conf = table.merge base_conf, {rng: map.rng}
                    vault = SourceMap.area_template_create(Vaults.hive_entrance_vault conf)
                    if not NewDungeons.place_feature(map, vault)
                        return nil
            return true
