...
function build_lanarts(){
    ...
    cd ../runtime && python compile_images.py > compiled/Resources.lua
    cd ..
}
...
function run_lanarts(){
    ...
    if handle_flag "--gdb" || handle_flag "-g" ; then
        ...
        gdb -silent -ex="break lua_error" -ex="break exit" -ex="break _exit" -ex="break abort" -ex="catch throw" -ex=r --args ../build/src/lanarts $args
    else
