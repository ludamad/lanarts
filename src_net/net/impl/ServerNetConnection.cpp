		...
		if (ss){
			...
			asio::async_read(
					...
					boost::bind(socketstream_read_header_handler, ss,
							asio::placeholders::error));
            sendlock.lock();
            std::vector<boost::shared_ptr<NetPacket> > tosend_copy = tosend_to_new;
            sendlock.unlock();
...
void ServerNetConnection::broadcast_packet(const NetPacket & packet, bool send_to_new) {
	streamlock.lock();
    ...
    if (send_to_new){
        sendlock.lock();
        tosend_to_new.push_back(boost::shared_ptr<NetPacket>(new NetPacket(packet)));
        sendlock.unlock();
    }
}
