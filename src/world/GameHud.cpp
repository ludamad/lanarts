...
static void fill_buff2d(char* buff, int w, int h, int x, int y, const Colour& col, int rw=2, int rh=2){
...
void GameHud::draw_minimap(GameState* gs, int subx, int suby){
	...
	for (int y = 0; y < tileh; y++){
		...
		for (int x = 0; x < tilew; x++){
			...
			if (tile == 0){/*floor*/
				...
				iter[0] = 0, iter[1] = 0, iter[2] = 0, iter[3] = 255;
			}
			iter += 4;
		}
	}
	...
	fill_buff2d(minimap_arr,tilew,tileh, arr_x - arr_x%2, arr_y - arr_y%2, Colour(255,0,0),2,2);
...
GameHud::GameHud(int x, int y, int width, int height) :
		x(x), y(y), _width(width), _height(height), bg_colour(0,0, 0), minimap_arr(NULL){
}
GameHud::~GameHud(){
	delete[] minimap_arr;
}
