...
struct Squares {
	...
	void set_region_with_perimeter(const Region& r, const Sqr& val,
		...
		Region rr = { r.x + padding, r.y + padding, r.w - padding * 2, r.h
				- padding * 2 };
...
static bool generate_room(MTwist& mt, int room_id, Squares& s, const Region& r,
		int padding) {
	if (!s.verify(r))
		return false;
	bool marked = (rand(mt,4) == 0);
	int mark = marked ? rand(mt,1,4) : 0;
	Sqr val = { true, false, false, mark, 0, room_id };
...
static void random_entrance(MTwist& mt, Region& r, Pos& p, bool& axis,
		bool& more, int len = 2) {
	int ind;
...
void generate_random_level(RoomgenSettings& rs) {
	...
	for (int id = 1; id <= target_rooms; id++) {
		for (int attempts = 0;; attempts++) {
			rx = rand(mt, w - 1 - padding) | 1, ry = rand(mt, h - 1 - padding)
					| 1;
			rw = rand(mt, minr, maxr), rh = rand(mt, minr, maxr);
			Region r = { rx, ry, rw, rh };
			Room room = { { rx + padding, ry + padding, rw - padding * 2, rh
					- padding * 2 }, 0 };

			if (generate_room(mt, id, s, r, padding)) {
				printf("Generated room x=%d,y=%d,w=%d,h=%d\n", rx, ry, rw, rh);
				room_list.push_back(room);
				break;
			}
			if (attempts > 10000) goto NoMoreRooms;
		}
	}
	NoMoreRooms:
	Pos p;
	...
	for (int r = 0; r < rs.max_tunnels; r++) {
	...
	for (int i = 0; i < room_list.size(); i++){
		const Region& r = room_list[i].room_region;
...
void floodfill(Squares& s, int y, int x, unsigned short groupID) {
	...
	for (i = -1; i <= 1; i++) {
		for (j = -1; j <= 1; j++) {
			...
			if (i + x < s.w && i + x >= 0 && j + y < s.h && j + y >= 0
