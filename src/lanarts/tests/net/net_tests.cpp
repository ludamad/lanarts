...
static void test_simple_client_server_msg() {
...
static void test_multiple_clients() {
	...
	server->send_message(msgsrv, strlen(msgsrv), NetConnection::ALL_RECEIVERS);
...
void net_unit_tests() {
	UNIT_TEST(test_simple_client_server_msg);
	UNIT_TEST(test_multiple_clients);
	UNIT_TEST(test_multiple_messages);
}
