...
namespace oxygine
{
                               ...
                               const RectF& source, const RectF& destination, const RenderState& rs)
    {
...
#endif
        {
            ...
            _renderer->draw(color, source, destination);
        }
    }
    // </LUDAMAD EDIT>
    ...
    // <LUDAMAD EDIT>
    void STDMaterial::doRender(Color color, const RenderState& rs, RectF destination, blend_mode mode)
    {
        Material::setCurrent(this);
        ...
        _renderer->setTransform(rs.transform);
        _renderer->draw(rs.getFinalColor(color), RectF(0, 0, 1, 1), destination);
    }
    // </LUDAMAD EDIT>
