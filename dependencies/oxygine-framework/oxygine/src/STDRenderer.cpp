...
namespace oxygine
{
    ...
    std::vector<unsigned short> STDRenderer::indices16_lines;
    ...
    void STDRenderer::initialize()
    {
        ...
        indices16_lines.reserve(3000 * 4);
        for (int t = 0; t < 3000; t += 1)
        {
            int i = t * 4;
            indices16_lines.push_back(i + 0);
            indices16_lines.push_back(i + 1);
            indices16_lines.push_back(i + 2);
            indices16_lines.push_back(i + 3);
        }
    ...
    void STDRenderer::drawLines(void* vertexData, size_t size)
    {
        size_t count = size / _vdecl->size;
        size_t indices = count;

        setBlendMode(blend_alpha);
        setTransform(Transform());
        setTexture(STDRenderer::white);
        preDrawBatch();
        getDriver()->draw(IVideoDriver::PT_LINE_LOOP, _vdecl, vertexData, (unsigned int)count, &indices16_lines.front(), (unsigned int)indices);
    }
